...
class F_IncomeInfoPage {
    ...
    constructor(page: Page) {
        ...
        this.fieldIncome = page.getByLabel('Monthly income (before tax):');
    ...
    async _enterIncome(income: string) {
    ...
    async happyPathPopulate(dataIn: string[]) {
        await this._enterIncome(dataIn[0]);
        await this.page.waitForTimeout(500);
        await this._selectPayFrequency(dataIn[1]); // hard coded to monthly temporarily
        await this.page.waitForTimeout(500);
        await this._enterLastPayDate(dataIn[2]);
        await this.page.waitForTimeout(500);
        await this._enterNextPayDate(dataIn[3]);
        await this.page.waitForTimeout(500);
        await this._NEXT();
    }

}
export default F_IncomeInfoPage;
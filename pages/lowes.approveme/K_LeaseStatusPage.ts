...
class K_LeaseStatusPage {
    ...
    async verifySuccessApproved() {
        await expect(this.headingCongrats).toBeVisible({timeout: 30000});
        await expect(this.headingApproved).toBeVisible({timeout: 30000});
        await expect(this.textApproved).toBeVisible({timeout: 30000});

...
test.describe('jared', async () => {
  ...
  test('Jared denied', { tag: ['@approveme', '@signet', '@jared', '@splashpage', '@happy', '@denied'] }, async () => {
    ...
    await expect(async () => {
      ...
      try {
        ...
        await cPage.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {status: 'passed', reason: 'jared denied'}})}`);
      }catch(Error) {
